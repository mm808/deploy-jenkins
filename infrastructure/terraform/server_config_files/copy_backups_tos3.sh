...
# script must get a bucket name as an argument ex) astech-jenkins-server-files-nonprod-sharedservices
...
# copy backup to s3
echo "Copying ${target_file} to ${1}/backups ..." >> $LOGFILE
aws s3 cp $jenkins_backups_dir/$target_file s3://$1/backups/$target_file
echo "Jenkins backup file upload complete: $(date)" >> $LOGFILE

echo "Copying sudoers to ${1}/backups ..." >> $LOGFILE
aws s3 cp $sudoers_path s3://$1/backups/sudoers.$current_date
